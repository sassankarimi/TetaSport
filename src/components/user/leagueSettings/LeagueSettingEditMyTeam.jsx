...
export class LeagueSettingEditMyTeam extends Component {
    static displayName = LeagueSettingEditMyTeam.name;
    ...
    addNewPlayerToTeam = (playerID, playerName, playerUserName, avatar) => {
        if (this.state.myTeam !== 0) {
            ...
            form.append('TeamID', this.state.myTeam);
            form.append('LeagueID', leagueIdDecoded);
            ...
            toast.error('تیم مورد نظر را انتخاب کنید', { position: toast.POSITION.TOP_CENTER });
        }
        document.getElementById('createTeam-search-result').style.opacity = 0;
        document.getElementById('createTeam-search-result').style.visibility = 'hidden';
        document.getElementById('createTeam-search-result').style.zIndex = -1;
    ...
    render() {
        // تایتل صفحه
        document.getElementsByTagName("title")[0].innerHTML = `تتااسپرت|ویرایش تیم ها من`;
        const { teams, exceptThesePlayers, searchPlayers } = this.state;
        let isTeamZero = true;
        if (teams.length !== 0) {
            isTeamZero = false;
        }
