...
export default class PlayerProfileLeagues extends Component {
    static displayName = PlayerProfileLeagues.name;
    constructor(props) {
        super(props)
        this.state = {
            joinedLeague: true,
    ...
    componentDidMount() {
        ...
        if (localStorage.getItem('view_profile') && localStorage.getItem('view_profile') !== 0) {
            ...
            fetch(`${configPort.TetaSport}${configUrl.GetUserLeagues}`, {
                method: 'POST',
                headers: {
                ...
                data => {
                    if (data) {
                        ...
                        if (data.LeaguesTypeMember.Leagues.length === 0 && data.LeaguesTypeForMe.Leagues.length === 0) {
                            this.setState({
                                Leagues: ' در هیچ لیگی شرکت نکرده است ',
                            });
                        }
                    }
                });
            ...
            this.setState({
                Leagues: 'خطایی رخ داده دوباره تلاش کنید',
            });
        }
    }
    ...
    render() {
        if (this.state.LeaguesTypeForMe.length || this.state.LeaguesTypeMember.length) {
            ...
            return (
                PlayerProfileLeagues.renderMyLeaguesAndJoinedLeagues(LeaguesImMember, LeaguesForMe)
            )
